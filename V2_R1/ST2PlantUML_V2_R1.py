...
def main():
    ...
    if not dev__:
        pouPath = input("Please Copy/Paste the *Absolute* path to a Twincat POU Directory.\nNOTE: This directory should contain at least 1 '.TcPou' file-type.\n>> ")
        try: 
            os.chdir(pouPath)
        except:
            print(f"\n\nCould not change to directory: {pouPath}\nPlease restart program to try again\nExiting...\n\n")
            return
        pouFiles = []
        for file in os.listdir():
            if ".TcPOU" in file:
                pouFiles.append("./"+ file )
                print(f"Found File : {pouFiles[-1]}")
            
        
    timeString = datetime.now().strftime("%Y-%m-%d_%H_%M_%S")
    if not "PlantUML_Output" in os.listdir(programPath):
        os.mkdir(programPath + f"/PlantUML_Output")
    outputDir = programPath + f"/PlantUML_Output/{timeString}" 
    os.mkdir(outputDir)
    
    if dev__:
        parseTcPouFile(GLOBALS_["TestTcPou"], f"output", outputDir)
    else:
        for file in pouFiles:
            pouName = file.replace('.TcPOU', '')
